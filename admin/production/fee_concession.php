...
if(!isLoggedIn()){
  ...
  if(isset($_POST['add'])){
   ...
   //  $date =$_POST['date'];
    ...
    $amount =  $_POST['amount'];
    $query = "INSERT INTO `fee_concession`  (`student_id`, `feemonth`, `year`, `fee_type`, `amount`) VALUES ('$student_id','$month','$year','$feetype','$amount') ";
          ...
          <div class="">
            <div class="page-title">
              <div class="title_left">
                <h3>Fee Concessions - </h3>  <h2 style="color:red"> <?php  if(isset($_GET['error'])){  $error= $_GET['error'];} echo $error; 
                  ?></h2>
              </div>
            </div>
...
$query = "SELECT * FROM `fee_concession` WHERE `id`='$edit_id' ";
                        $result = mysqli_query($link, $query);
                        while($row = mysqli_fetch_array($result)){
 ?>
...
<div class="col-xl-8 col-md-8 col-xs-12">
  ...
  <?php  if(isset($_GET['id'])){
   ...
   $query ="DELETE FROM `fee_concession` WHERE `id`='$id'  ";
    $result = mysqli_query($link, $query); 
 if($result==TRUE){
...
?>
 ...
 <?php        
               if(isset($_POST['edit'])){
                     ...
                     $year = $splitValues[1];
                      $amount =  $_POST['amount'];
                      $query4 = "UPDATE `fee_concession` SET `feemonth`='$month', `year`='$year', `fee_type`='$feetype',`amount`='$amount' WHERE `id`='$staff'";
                      $result4 = mysqli_query($link, $query4);
                      
                      if ($result4 == TRUE) {
                          $error = "Updated Successfully";
                          $url = 'fee_concession.php?error=' . urlencode($error) . '&student_id=' . urlencode($student_id);
      ...
      if(isset($_GET['student_id'])){                        
                ...
                $student_id=0;    
                    ...
                    <table id="example" class="table table-striped jambo_table bulk_action">
                        ...
                        $query2 = "SELECT * FROM `fee_concession` WHERE `student_id`='$student_id' ORDER BY id ASC";
...
$count=1;
                        ...
                        ?>
                          <tr class="even pointer">
                            ...
                            <td class=" "><button onclick="location.href='fee_concession.php?edit_id=<?php echo $row2['id']; ?>&student_id=<?php echo $student_id; ?>'" class="btn"><i class="fa fa-edit"></i> </button>
                            <button onclick="location.href='fee_concession.php?id=<?php echo $row2['id']; ?>'" class="btn"><i class="fa fa-trash"></i> </button>
                            </td> 
                          </tr> 
