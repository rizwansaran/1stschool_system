...
$query ="UPDATE `fee` SET `amount`='$amount', `tamount`='$tamount', `date`='$d' WHERE `studentid`='$id' AND `challan_no`='$total'";
 ...
 if($result=="TRUE"){
          $error = "Updated Successfully";
header("location:active_students.php?msg=Fee Updated Successfully");
exit();

 }
else {
  $error = "Error While Updating...";
header("location:active_students.php?msg=Error While Updating Fee...");
}
...
<?php  if (!isset($_GET['sid'])) { ?>
                    ...
                    <form id="demo-form" data-parsley-validate action="addfee2.php" method="post" enctype="multipart/form-data">
...
?>
                    ...
                    if (isset($_GET['sid'])) {
                       ...
                       if($result && mysqli_num_rows($result)){
                        ...
                        <br><br><br>
                          <form action="addfee2.php" method="POST">
...
{  
 ...
 $query2 = "SELECT * FROM `fee` WHERE `studentid` = '$id' AND feemonth= '$batch1' AND feeyear= '$year1'  ";
                       $result2 = mysqli_query($link, $query2);
...
else
{
 ...
 $query2 = "SELECT * FROM `fee1` WHERE `studentid` = '$id' AND feemonth= '$batch1' AND feeyear= '$year1'  ";
                       $result2 = mysqli_query($link, $query2);
                       if($result2 && mysqli_num_rows($result2) > 0){
                       $fe = mysqli_fetch_array($result2);
...
else
{
  ...
  $query2 = "SELECT * FROM `chalan` WHERE studentid = '$id' AND feemonth= '$batch1' AND year= '$year1'";
                     $result2 = mysqli_query($link, $query2);
...
$class=$student['class'];
  ...
  $query2 = "SELECT * FROM `classchalan` WHERE class = '$class' AND feemonth= '$batch1' AND year= '$year1' ";
                       $result2 = mysqli_query($link, $query2);
                       if(mysqli_num_rows($result2) > 0){
                        $fe = mysqli_fetch_array($result2);
